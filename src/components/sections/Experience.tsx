...
import { Calendar, Briefcase, ChevronRight, X, Award, Code2 } from "lucide-react";
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-20 sm:py-28 relative overflow-hidden bg-[#FDFBF7] text-[#2D1217] border-b border-[#E8DFC8]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-16 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-6 relative pt-4">
            {/* Vector Road Path Line (Desktop & Mobile) */}
            <div className="absolute left-[39px] sm:left-[49px] top-6 bottom-6 w-3 bg-[#E8DFC8] rounded-full z-0" />
                ...
                return (
                  <motion.div
                    ...
                    className={`flex items-start gap-4 sm:gap-6 p-4 sm:p-5 rounded-2xl border cursor-pointer transition-all ${
                      isSelected
                        ? "bg-[#FFFDF9] border-[#D96B43] shadow-md translate-x-2"
                        : "bg-[#F5EFE6]/60 border-[#E8DFC8] hover:border-[#D96B43]/60 hover:bg-[#FFFDF9]"
                    }`}
                  >
                    ...
                    <div
                      className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center font-bold text-sm sm:text-base shrink-0 shadow-sm transition-colors ${
                        isSelected
                          ...
                          : "bg-[#FFFDF9] border border-[#E8DFC8] text-[#4A1D24]"
                      }`}
                    >
                      <span>0{index + 1}</span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="font-editorial text-lg sm:text-xl font-bold text-[#4A1D24] truncate">
                        {exp.position}
                      </h3>
                      <p className="text-xs text-[#2D1217]/70 font-semibold truncate mt-0.5">
                        {exp.company}
                      </p>
                    </div>
                    ...
                    <ChevronRight
                      className={`w-5 h-5 transition-transform my-auto shrink-0 ${
                        isSelected ? "text-[#D96B43] translate-x-1" : "text-[#4A1D24]/40"
                      }`}
                    />
                  </motion.div>
                );
            ...
            <AnimatePresence mode="wait">
              {selectedExp ? (
                <motion.div
                  ...
                  className="p-6 sm:p-8 rounded-3xl bg-[#FFFDF9] border border-[#E8DFC8] shadow-lg space-y-6"
                >
                  <div className="flex items-start justify-between border-b border-[#E8DFC8] pb-5">
                    <div>
                      <span className="text-xs font-bold text-[#D96B43] uppercase tracking-widest block mb-1">
                        {selectedExp.duration}
                      </span>
                      <h3 className="font-editorial text-2xl sm:text-3xl font-bold text-[#4A1D24]">
                        {selectedExp.position}
                      </h3>
                      <p className="text-sm font-semibold text-[#2D1217]/80 mt-1 flex items-center gap-2">
                    ...
                    <div className="space-y-3 pt-2">
                      ...
                      <ul className="space-y-2">
                        {selectedExp.achievements.map((ach, i) => (
                          <li key={i} className="flex items-start gap-2.5 text-xs sm:text-sm text-[#2D1217]/80">
                    ...
                    <div className="space-y-3 pt-3 border-t border-[#E8DFC8]">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedExp.techStack.map((tech, i) => (
                          <span
                            key={i}
                            className="px-3 py-1 rounded-xl bg-[#F5EFE6] border border-[#E8DFC8] text-xs font-semibold text-[#4A1D24]"
                          >
                            {tech}
                          </span>
                        ))}
